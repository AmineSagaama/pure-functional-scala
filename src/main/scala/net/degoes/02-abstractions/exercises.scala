...
object algebra {
    ...
    new Semigroup[NotEmpty[A]] {
      override def append(f1: NotEmpty[A], f2: => NotEmpty[A]): NotEmpty[A] =
        f1.tail match {
          ...
          case Some(tail) => NotEmpty(f1.head, Some(append(tail, f2)))
        }
    }
  ...
  // we have to specify that A is a Semigroup because it's * => *
  // A is a semigroup so we can use |+| to combine two options
  //
  ...
  // String semigroup
  implicit def SemigroupString[String]: Semigroup[String] =
  ...
  implicit val DisjMonoid: Monoid[Disj] =
    new Monoid[Disj] {
      override def zero: Disj = Disj(false)

      override def append(f1: Disj, f2: => Disj): Disj = {
        Disj(f1.value || f2.value)
      }
    }
  ...
  def SemigroupMap[K, V: Semigroup]: Semigroup[Map[K, V]] =
    new Semigroup[Map[K, V]] {
      override def append(f1: Map[K, V], f2: => Map[K, V]): Map[K, V] =
        (f1.keys ++ f2.keys).foldLeft[Map[K, V]](Map()) {
  ...
  object Capability {
    ...
    case class UserPermission(value: Map[AccountID, Map[ResourceID, Set[Capability]]])
  ...
  implicit val MonoidUserPermission: Monoid[UserPermission] =
    new Monoid[UserPermission] {
      override def zero: UserPermission = UserPermission(Map())

      override def append(f1: UserPermission, f2: => UserPermission): UserPermission =
        UserPermission(f1.value |+| f2.value)
    }
...
object functor {
  ...
  object Zip {
    implicit val ZipOption: Zip[Option] =
      new Zip[Option] {
        ...
        // very powerful !! you can do concurrency with zip for example
  ...
  /*
   trait Monad[F[_]] extends Applicative[F] {
      ...
      // the most powerful is bind
      def bind[A, B](fa: F[A])(f: A => F[B]): F[B]
  }

  exp :

  def geolocationUser(ip: IPAddress): Future[Geolocation]
  def updateUserProfile(userID: UserID, geolocation: Geolocation): Future[UpdateResult]

  Monad[Future].bind(geolocationUser(ip))(geolocation =>
  updateUserProfile(userId, geolocation)
  ): Future[UpdateResult]
   */
  ...
  case class Address(postalCode: Option[Int])

  val employee: Employee = ???
  val postalCode: Option[Int] = employee.manager.flatMap(person =>
